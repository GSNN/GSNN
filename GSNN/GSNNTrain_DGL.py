...
class GSNNTrainer():
    ...
    def train(self, adj, feature, label, train_mask, val_mask, test_mask, epochs, earlystopping=False):
        ...
        for epoch in range(epochs):
            self.gsnn.train()
            self.gsnn.training=True
#             if earlystopping:
            self.optimizer.zero_grad()
#             elif epoch%50>20:
#                 self.gsnn.x_to_yu.eval()
...
#                 self.optimizer1.step()
            ...
            if epoch % self.print_freq == 0:
                ...
                print("Reconstruct: {}, Kl: {}, kl_pq:{}, q_yl:{}.".format(reconstruct_total.item(), kl_total.item(), kl_pq.item(), q_yl.item()))
                print('-'*50)
        ...
        print("Val_mean_acc: {}, Val_max_acc: {}".format(best_mean_acc, best_max_acc))
#         max_acc, mean_acc, y_p_max, y_p_mean = self.evalGSNN(adj, feature, label, test_mask, sample_num=40)
        max_acc, mean_acc, y_p_max, y_p_mean, t = self.evalGSNN_test(adj, feature, label, val_mask, test_mask, 1, non_label)
        print("Test_mean_acc: {}, Test_max_acc: {}, q_max: {}, q_min: {}".format(mean_acc, max_acc, y_p_max, y_p_mean))
        print(t)
        print('-'*50)
        return t
    ...
    def evalGSNN_test(self, adj, feature, label, val_index_mask, test_index_mask, sample_num, non_label):
        self.gsnn.eval()
        self.gsnn.training=False
        sample_list = []
        test_sample_list = []
        y_p_list = []
        total_size = len(test_index_mask)
        sample_total = torch.zeros_like(label, dtype=torch.float32)
        l = []
        for i in range(sample_num):
            y_eval, y_p = self.gsnn(feature, label, adj, non_label)
            ...
            y_p_scalar = y_p.argmax(dim = 1)
            label_scalar = label.argmax(dim = 1)
            sample_list.append((y_eval_scalar[val_index_mask]==label_scalar[val_index_mask]).sum().item())
            test_sample_list.append((y_eval_scalar[test_index_mask]==label_scalar[test_index_mask]).sum().item())
            y_p_list.append((y_p_scalar[test_index_mask]==label_scalar[test_index_mask]).sum().item())

        sample_avg = sample_total / sample_num
        sample_scalar = sample_avg.argmax(dim = 1)
        idx = np.where(np.argsort(sample_list)>=(len(sample_list)-20))[0]
        print(idx)
        print()
        sample_total_test = torch.zeros_like(label, dtype=torch.float32)
        for idxx in idx:
            sample_total_test = sample_total_test + l[idxx]
        sample_avg_total = sample_total_test / len(idx)
        sample_scalar_total = sample_avg_total.argmax(dim = 1)

        return float(max(test_sample_list)) / total_size, float((sample_scalar[test_index_mask]==label_scalar[test_index_mask]).sum().item()) / total_size, float(max(y_p_list)) / total_size, float(np.mean(y_p_list)) / total_size, float((sample_scalar_total[test_index_mask]==label_scalar[test_index_mask]).sum().item()) / total_size
